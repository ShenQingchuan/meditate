...
export class Calterm {
  ...
  public print = (customizeDay?: (str: string, day: Dayjs) => string) => {
    const daysAbbr = Calterm.daysOfWeek.map(d => d.slice(0, 2))
    const blank = []
    ...
    header = textTable([[...blank, header]], {hsep: ''})

    const firstDay = this.getFirstDayOfMonth().slice(0, 2)
    const grid = []
    const today = Number.parseInt(this.dayjs.format('D'), 10)
    ...
    const startDay = daysAbbr.indexOf(firstDay)
